...
public class BiasTest {
    ...
    public static void setupCandidates() {
        ...
        allCandidates =
                (LinkedList<Candidate>) parser.parse(
                        Candidate.class, filepath, ',', true);

        /*
        if the filename is the correlated variable, we're not looking at
        the gender variable at all while looking at different Candidate objects
       ...
       BiasTest.candidates =
             new ListObjsData<Candidate>(canAttr, allCandidates);
    }
    ...
    public static void main(String[] args) {
        ...
        newCandMale = (LinkedList<Candidate>)
                parser.parse(Candidate.class,
                        "data/testing_cis_male_correlated.csv",
                        ',',
                        true);
        ...
        /**
         * TODO: While testing correlated variables, change filepath from
         * "data/testing_cis_female.csv" to
        ...
        /*
         loop to test female and male hiring ratios with the same test files
          but large number of times to determine a better ratio by volume
         */
        for (int j = 0; j < 10; j++) {
            // build the tree
            TreeGenerator<Candidate> builder =
                    new TreeGenerator<Candidate>(BiasTest.candidates);
...
//            builder.printTree();
            ...
            for (int i = 0; i < newCandMale.size(); i++) {
                if (builder.lookupRecommendation(
                        newCandMale.get(i)).equals(Boolean.TRUE)) {
                    hiredYes++;
                }
            }
            ...
            for (int i = 0; i < newCandFemale.size(); i++) {
                if (builder.lookupRecommendation(newCandFemale.get(i)).
                        equals(Boolean.TRUE)) {
                    hiredYes1++;
                }
            }
            ...
            /*
            update the highest distances between male and female hired ratios
             and male and female ratios respectively
             */
            if (distance < (hiredYes / newCandMale.size() -
                    hiredYes1 / newCandFemale.size())) {

                distance = hiredYes / newCandMale.size() -
                        hiredYes1 / newCandFemale.size();
